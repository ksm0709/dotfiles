...
int main(int argc, char **argv) {
  ...
  ros::ServiceClient client =
      nh.serviceClient<rtabmap_ros::SetGoal>("set_goal");
  ...
  ros::Subscriber rviz_goal_sub =
      nh.subscribe("/rviz_goal", 10, rvizGoalCallback);
  ros::Subscriber vel_sub = nh.subscribe("/cmd_vel", 10, cmdvelCallback);

  ros::Publisher pose_pub =
      nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/amcl_pose", 10);
  ros::Publisher goal_reached_pub =
      nh.advertise<std_msgs::String>("/arrived_status", 1);
  ros::Publisher state_pub = nh.advertise<std_msgs::String>("/robot_state", 3);
  ros::Publisher rviz_goal_pub =
      nh.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 3);
    ...
    if (goal_msg_updated) {
      ...
      /*
                              ...
                              if( client.call(goal_srv) )
                              {
                                      ...
                                      int path_size =
         goal_srv.response.path_poses.size() - 1;
                                      rviz_goal_msg.header.stamp=ros::Time::now();
                                      rviz_goal_msg.header.frame_id = "map";
                                      ...
                                      rviz_goal_msg.pose =
         goal_srv.response.path_poses[path_size];
                                      rviz_goal_pub.publish(rviz_goal_msg);

                                      ROS_INFO("Goal request status :
         Sucessed");
                              }
                              else
