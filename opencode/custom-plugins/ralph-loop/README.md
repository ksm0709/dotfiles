...
- **키워드 기반 활성화**: 사용자 메시지에 "ralph" 키워드가 포함될 때만 플러그인이 활성화됩니다.
- **키워드 기반 프롬프트 인젝션**: "ralph" 키워드가 포함된 메시지에 자동으로 작업 완료 지침(Promise Word 출력)을 주입합니다.
- **조건부 Idle 상태 감지**: ralph-loop이 활성화된 세션에서만 동작하며, 에이전트가 작업을 마쳤을 때 약속된 단어(Promise Word)가 포함되어 있는지 확인합니다.
...
1. 오픈코드를 실행하고 일반적인 작업을 시작합니다.
2. **플러그인 활성화**: 작업 중 "ralph" 키워드(대소문자 무관)를 포함하여 메시지를 보내면 플러그인이 활성화되고 **"[Ralph Loop 플러그인] 모든 작업이 완료되면 반드시 `promiseWord`를 출력하세요."** 라는 지시사항이 자동으로 추가됩니다.
   - 예시: "ralph에게 상태 보고해줘", "RALPH에게 물어봐", "이것은 ralph의 작업입니다"
3. **루프 동작**: 활성화된 세션에서 에이전트가 작업을 수행하다가 `idle` 상태가 되었을 때, 메시지에 `promiseWord`가 없으면 플러그인이 개입하여 자동 루프를 시작합니다.
4. 플러그인은 현재 세션을 요약하여 `.opencode/sessions/(session-id)/ralph_summary.md`에 저장하고, 새 세션을 생성하여 요약본과 함께 **"[Ralph Loop 플러그인]"** 표시와 함께 작업을 계속하도록 지시합니다.
5. **반복 주입**: 활성화된 세션에서는 이후에도 "ralph" 키워드를 포함할 때마다 프롬프트가 다시 주입됩니다.

### 메시지 형식
- **활성화 및 주입**: `[Ralph Loop 플러그인] 모든 작업이 완료되면 반드시 'DONE'을 출력하세요.`
- **세션 재시작 시**: `[Ralph Loop 플러그인] 모든 작업이 완료되면 반드시 "DONE"을 출력하세요.`

### 키워드 기반 동작 방식
- **트리거**: 사용자 메시지에 "ralph" 키워드 포함 (대소문자 무관)
- **동작**: 
  1. 세션 활성화 및 프롬프트 인젝션 즉시 실행
  2. 해당 세션에서 ralph-loop 기능 활성화 (idle 상태 감지, 자동 루프 등)
- **목적**: 필요할 때만 선택적으로 ralph-loop 기능을 활성화하여 불필요한 개입 방지
