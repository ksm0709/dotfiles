...
class LoopNode extends BaseNode_1.BaseNode {
    async execute(_sessionId) {
        ...
        if (iteration >= maxIterations) {
            await this.blackboard.set(`${this.nodeName}_iteration`, 0);
            return this.result("max_reached", `Max iterations (${maxIterations}) reached`);
        }
        // 반복 카운터 증가
        await this.blackboard.set(`${this.nodeName}_iteration`, iteration + 1);
        // 'continue' 결과 반환 - routes에서 body_node로 연결
        return this.result("continue", `Iteration ${iteration + 1} of max ${maxIterations}`);
    }
    /**
     * 조건 평가
     */
