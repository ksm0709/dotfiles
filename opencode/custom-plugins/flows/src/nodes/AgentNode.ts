...
import type { NodeResult, AgentNodeConfig, AgentResultDef } from "../types/schemas";
...
export class AgentNode extends BaseNode {
   ...
   */
  private buildContextPrompt(basePrompt: string): string {
    // 개선된 히스토리 빌드 메서드 사용
   ...
   * 에이전트 실행
   */
  private async executeAgent(sessionId: string, prompt: string): Promise<void> {
    try {
      ...
      const response = lastMessage?.parts.find((p: any) => p.type === "text")?.text || "";
   ...
   * + 데이터 추출 문법 안내
   */
  private buildResultGuide(): string {
    ...
    if (Array.isArray(results)) {
      // 기존 배열 형식
      resultOptions = (results as AgentResultDef[])
...
---`;
   ...
   * 응답에서 결과 및 데이터 파싱
   */
  private async parseResult(response: string): Promise<NodeResult> {
   ...
   */
  private async extractData(response: string): Promise<Record<string, any>> {
   ...
   * 유효한 결과 이름 목록 조회
   */
  private getValidResultNames(): string[] {
    const results = this.config.results || {};
    
    if (Array.isArray(results)) {
      return (results as AgentResultDef[]).map(r => r.name);
    } else {
