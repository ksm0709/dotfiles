...
def process_files(filenames: list[str]) -> None:
    with ExitStack() as stack:
        files = [
            stack.enter_context(open(fn, encoding="utf-8"))
            for fn in filenames
        ]
...
def process_user(user_id: int) -> None:
    # Per GDR guidelines, pass format args separately to logging functions rather than pre-formatting.
    # GDR preferred: let the logger format the string
    logger.info("Processing user %s", user_id)
    try:
        # Process...
        logger.debug("User data loaded for %s", user_id)
    except ValueError:
