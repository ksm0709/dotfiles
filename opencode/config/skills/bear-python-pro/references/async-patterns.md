...
## GDR Compliance Notes

- Use direct iteration (not indices) in async for-loops
- Avoid local boolean flags in async control flow; prefer early returns or extracted functions
- Use guard clauses to reduce indentation in async functions
- Use `if` instead of `elif` after `return` in async functions
