...
## GDR Unit Test Triggers

Python readability reviewers should recommend writing unit tests when:

- A new package, module, class, method, or function has been added
- The MR creator is the code owner of the updated components

Writing tests helps improve readability by making code easier to refactor and test.

Reference: Bear Robotics unit testing wiki and coding sessions.
...
@pytest.fixture
def user_factory() -> Callable[[str, bool], User]:
...
def test_user_names(user_factory: Callable[[str, bool], User], name: str) -> None:
...
def test_sorted_is_ordered(lst: list[int]) -> None:
    ...
    for a, b in zip(sorted_lst, sorted_lst[1:]):
        assert a <= b
...
@pytest.fixture
def user_factory(db_session: Session) -> Generator[Callable[..., User], None, None]:
