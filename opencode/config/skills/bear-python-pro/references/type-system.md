...
All snippets in this guide follow Google Python Style Guide conventions and Bear Robotics GDR Python Review Guidelines.
...
def add_numbers(a: NumT, b: NumT) -> float:
...
### Accept Abstract (Generic)

- Accept abstract/generic types for loose coupling, easy testing, and more useful routines

Don't example:

```python
def update(robot_name_list_path, db_addr, table_name):
    with open(robot_name_list_path) as f:
        lines = f.readlines()
        db = my_db.connect(db_addr)
        cli = db.get_client()
        table = cli.get_table(table_name)
        for line in lines:
            table.add_row(line)
    cli.close()
```
...
```python
def update(robot_names, table):
    for robot_name in robot_names:
        table.add_row(robot_name)
```

Reference Feature Envy: https://refactoring.guru/smells/feature-envy

### Return Concrete

- When method name implies boolean (e.g., `has_current_map`), return concrete `bool`
- Don't: `return self.location and self.location.current_map`
- Do: `return bool(self.location and self.location.current_map)`

## Advanced Type Features

```python
from typing import Any, Literal, NotRequired, Self, TypeAlias, TypedDict, overload
...
# Explicit default handling without type ignores
def get_value(values: Mapping[str, T], key: str, default: T | None = None) -> T:
    if key in values:
        return values[key]
    if default is None:
        raise KeyError(key)
