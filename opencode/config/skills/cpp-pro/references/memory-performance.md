...
void SmartPointerExample() {
    std::unique_ptr<Resource> owned_resource = CreateResource();
    ...
    std::shared_ptr<Node> shared_node = node->GetShared();
    (void)owned_resource;
    (void)weak;
    (void)file;
    (void)shared_node;
...
float SimdSum(const float* data, size_t size) {
...
void FmaOperation(float* result, const float* a, const float* b,
...
// Prefetching
void ProcessWithPrefetch(const int* data, size_t size) {
...
std::string BuildString(bool condition) {
    std::string result;
    if (condition) {
...
// Aligned allocation
void* AlignedAllocWrapper(size_t alignment, size_t size) {
    void* ptr = nullptr;
