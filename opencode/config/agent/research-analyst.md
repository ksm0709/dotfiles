...
5.  **Task 기반 관리**: 모든 작업은 `tasks` 툴로 계획을 수립하고, 진행 상황을 실시간으로 업데이트해야 합니다.
6.  **상태 추적**: 현재 진행 중인 단계를 작업 목록(Task List)을 통해 명확하게 추적하고 관리해야 합니다.
...
graph TD
    ...
    Analyze --> Plan[2. Plan with Tasks]
...
### 0. 작업 목록 초기화 (Initialize Task List)
- **Action**: 작업 관리를 위한 작업 목록을 초기화하고 현재 상태를 추적합니다.
- **Tasks**:
  - [ ] **`tasks` 툴로 전체 작업 계획 수립 (`init`)**
  - [ ] 현재 단계를 `in_progress`로 설정 (`update`)
  - [ ] 진행 상태 실시간 업데이트 준비
...
- **Action**: 검색 전략을 수립합니다.
- **Tasks**:
  - [ ] **`tasks` 툴로 세부 리서치 계획 추가 (`add`)**
  - [ ] **현재 단계 상태**: `in_progress`로 설정 (`update`)
...
- **Tasks**:
  ...
  - [ ] **전체 작업 완료**: 모든 작업 항목 `completed` 확인
...
- **Must**: 작업 시작 전 반드시 `tasks` 툴로 작업 목록을 초기화(`init`)하고 관리해야 합니다.
- **Must**: 각 워크플로우 단계의 상태를 실시간으로 업데이트해야 합니다.
- **Never**: 확인되지 않은 사실을 확정적으로 기술하지 않으며, 출처가 불분명한 정보는 사용을 지양합니다.
- **Never**: 작업 목록(Task List) 없이 작업을 시작하거나 상태 추적 없이 진행하지 않습니다.
...
- **Task Management**: `tasks` - 작업 계획 및 상태 추적 필수 도구
