...
**위임 시 주의사항**:
- 에이전트 호출 시 **OpenSpec의 관련 부분(시나리오, 태스크)**을 명확히 전달하세요.
- "알아서 해주세요"가 아닌, "이 시나리오를 통과하는 테스트를 작성해주세요"와 같이 구체적으로 지시하세요.

---

## 스펙 주도 테스팅 표준 (Spec-Driven Testing Standards)

모든 프로젝트는 **TDD (Test-Driven Development)**를 원칙으로 합니다.
...
3.  **커버리지**: 모든 시나리오에 대한 테스트가 존재해야 하며, 엣지 케이스(예외 상황)를 반드시 포함해야 합니다.
4.  **독립성**: 각 테스트는 서로 의존하지 않고 독립적으로 실행 가능해야 합니다.
...
**제안서 구성 요소**:
- **proposal.md**: *Why* & *What*. 비즈니스 가치와 변경 범위.
- **design.md**: *How*. 아키텍처, DB 스키마, API 설계.
- **tasks.md**: *Plan*. 실행 가능한 작업 목록.
- **spec deltas**: *Requirements*. 구체적인 변경 사항과 검증 시나리오.

**검증 기준**:
- 변경 범위가 요청 사항에 딱 맞게(Tightly Scoped) 설정되었는가?
- 모든 요구사항에 검증 가능한 시나리오가 있는가?
- 태스크가 완료 여부를 명확히 알 수 있게 정의되었는가?
...
**기술적 의사결정 체크리스트**:
- [ ] **비즈니스 가치**: 이 작업이 프로젝트 목표에 기여하는가?
- [ ] **유지보수성**: 팀이 이 코드를 이해하고 관리할 수 있는가?
- [ ] **확장성**: 미래의 변경을 어렵게 만들지 않는가? (Over-engineering 주의)

**품질 보증(QA) 기준**:
- 모든 CI 체크(Lint, Format, Type) 통과
- 테스트 스위트 100% 통과
- 불필요한 주석 및 디버깅 코드 제거
- 명확한 변수명과 함수명 사용

**회피해야 할 안티패턴**:
- ❌ **Spec 없는 구현**: 승인된 제안서 없이 코드를 작성하는 행위
- ❌ **테스트 건너뛰기**: "간단한 수정이니까"라며 테스트를 생략하는 행위
- ❌ **무단 커밋**: 사용자의 확인 없이 코드를 커밋/푸시하는 행위
- ❌ **과도한 엔지니어링**: 현재 요구사항을 넘어서는 복잡한 추상화 도입

**에스컬레이션 (사용자 개입 요청)**:
- 요구사항이 불명확하거나 상충될 때
- 예상치 못한 기술적 난관으로 일정이 크게 지연될 때
- 보안이나 데이터 손실 위험이 발견되었을 때
