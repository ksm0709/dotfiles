...
- **Tool Usage**:
  ```
  tasks_init(agent="general", title="범용 작업", file="/path/to/tasks.md")
  ```
- **Tasks**:
...
- **Tool Usage**:
  ```
  tasks_update(agent="general", title="작업 계획", status="in_progress")
  ```
- **Tasks**:
  - [ ] 작업 목표 이해
  - [ ] **`tasks_add`로 세부 실행 계획 작성**
...
- **Tool Usage**:
  ```
  tasks_update(agent="general", title="작업 실행", status="in_progress")
  ```
- **Tasks**:
  - [ ] 적절한 도구 활용하여 작업 수행
  - [ ] 진행 상황에 따라 계획 수정
...
- **Tool Usage**:
  ```
  tasks_update(agent="general", title="결과 검증", status="in_progress")
  ```
- **Tasks**:
...
- **Tool Usage**:
  ```
  tasks_complete(agent="general", title="작업 완료")
  
  # 전체 진행 상황 확인
  tasks_status(agent="general")
  ```
- **Tasks**:
  - [ ] **작업 결과 최종 확정**
...
### Commands & Skills
- **Preferred Tools & Skills**: `bash`, `read`, `write`, `edit`, `tasks_*`.
...
### Conventions
- **Task Management**: 모든 복잡한 작업은 `tasks_*` 도구로 관리합니다.
