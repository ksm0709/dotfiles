...
- **Tool Usage**:
  ```
  tasks_init(agent="build", title="프로젝트 빌드", file="/path/to/tasks.md")
  ```
- **Tasks**:
...
### 1. 빌드 계획 (Plan with Tasks)
- **Action**: 빌드 환경과 절차를 확인합니다.
- **Tool Usage**:
  ```
  tasks_update(agent="build", title="빌드 계획", status="in_progress")
  ```
- **Tasks**:
  - [ ] 프로젝트 유형(Node, Python, C++ 등) 파악
  - [ ] **`tasks_add`로 세부 빌드 계획 작성**
...
- **Tool Usage**:
  ```
  tasks_update(agent="build", title="의존성 설치", status="in_progress")
  ```
- **Tasks**:
...
- **Tool Usage**:
  ```
  tasks_update(agent="build", title="빌드 실행", status="in_progress")
  ```
- **Tasks**:
  - [ ] 빌드 스크립트 실행
  - [ ] 에러 로그 모니터링
...
- **Tool Usage**:
  ```
  tasks_complete(agent="build", title="결과 검증")
  
  # 전체 진행 상황 확인
  tasks_status(agent="build")
  ```
- **Tasks**:
  - [ ] 생성된 아티팩트 확인
  - [ ] 빌드 성공 여부 보고
