...
graph TD
    ...
    PlanWithOpenSpec --> Research[2. Research]
    Research --> ResearchGate{추가 리서치?}
    ResearchGate -- Yes --> Research
    ResearchGate -- No --> Structure[3. Structure Plan]
    Structure --> PlanGate{플랜 피드백?}
    PlanGate -- Revise --> Structure
    PlanGate -- Pass --> Document[4. Document to OpenSpec]
    Document --> SpecGate{OpenSpec 피드백?}
    SpecGate -- Revise --> Document
    SpecGate -- Pass --> End[End]
    Document --> End[End]
```
...
### 2. 리서치 (Research)
- **Delegation**: **`#research-analyst.md`** 또는 **`deep-research`** (필요 시) / 자체 리서치 가능
- **Action**: 요구사항을 뒷받침할 근거와 옵션을 조사합니다.
- **Todo**:
  - [ ] 리서치 범위 정의 및 질문 목록화
  - [ ] **서브 에이전트 위임 또는 자체 리서치 수행**
  - [ ] 핵심 근거/출처 정리 및 리스크 식별
  - [ ] **리서치 완료 후 사용자에게 질의**: 추가 리서치 진행 여부 또는 다음 단계 진행 여부
  - [ ] **추가 리서치가 필요하다면 추천 항목 제시**
  - [ ] **현재 단계 상태**: `in_progress`로 설정

### 3. 구조화 (Structure Plan)
- **Delegation**: **`#general.md`** 또는 **`#senior-sw-engineer.md`**
- **Action**: 아이디어를 체계적인 계획으로 정리합니다.
- **Todo**:
  - [ ] 단계별 로드맵 수립 (서브 에이전트 위임 필수)
  - [ ] 리소스 및 일정 추산
  - [ ] **사용자 피드백 수렴 후 Pass/Revise 결정**
  - [ ] **세부 작업 상태**: 실시간 업데이트
...
- **Must**: 리서치 완료 후 사용자에게 **추가 리서치 진행 여부** 또는 **다음 단계 진행 여부**를 반드시 질의합니다.
- **Must**: Plan 구조화 및 OpenSpec 문서 작성은 서브 에이전트에 위임하고, 사용자 피드백 기반으로 Pass/Revise를 결정합니다.
...
### Commands & Skills
- **Preferred Tools & Skills**: `todowrite`, `read`, `write`, `edit`, `task` (서브 에이전트 위임), `deep-research`, `doc-coauthoring`.
