...
- **Tool Usage**:
  ```
  tasks_init(agent="plan", title="계획 수립", file="/path/to/tasks.md")
  ```
- **Tasks**:
...
### 1. 목표 분석 (Plan with Tasks)
- **Action**: 프로젝트의 목표와 요구사항을 파악합니다.
- **Tool Usage**:
  ```
  tasks_update(agent="plan", title="목표 분석", status="in_progress")
  ```
- **Tasks**:
  - [ ] 핵심 목표 정의
  - [ ] **`tasks_add`로 세부 기획 계획 작성**
...
- **Tool Usage**:
  ```
  tasks_update(agent="plan", title="브레인스토밍", status="in_progress")
  ```
- **Tasks**:
  - [ ] 가능한 솔루션 나열
  - [ ] 장단점 분석
...
- **Tool Usage**:
  ```
  tasks_update(agent="plan", title="구조화", status="in_progress")
  ```
- **Tasks**:
  - [ ] 단계별 로드맵 수립
  - [ ] 리소스 및 일정 추산
...
- **Tool Usage**:
  ```
  tasks_update(agent="plan", title="문서화", status="in_progress")
  ```
- **Tasks**:
  - [ ] 명확하고 설득력 있는 문서 작성
  - [ ] 이해관계자 검토 요청
...
- **Tool Usage**:
  ```
  tasks_complete(agent="plan", title="계획 문서화")
  
  # 전체 진행 상황 확인
  tasks_status(agent="plan")
  ```
- **Tasks**:
  - [ ] **계획 결과 최종 확정**
...
### Commands & Skills
- **Preferred Tools & Skills**: `tasks_*`, `read`, `task` (리서치 위임).
