...
import sys
import logging
from opencode_memory.utils.logging import get_client_logger
...
def cmd_get_compaction_context() -> str:
    ...
    cm = get_manager()
    logger.info("Requesting compaction context from server...")
    context = cm.get_compaction_context()
    logger.info(f"Received compaction context ({len(context)} chars)")
    return context
...
def main():
    ...
    try:
        if args.command == "init":
            ...
            cm = get_manager()
            # Map CLI args to ContextManager methods
            # Note: ContextManager methods return None or void-like, 
            # but we want to confirm success.
            if args.type == "change":
                cm.record_change(args.content, args.file)
            elif args.type == "read":
                cm.record_read(args.content, args.file)
            elif args.type == "decision":
                cm.record_decision(args.content)
            elif args.type == "error":
                cm.record_error(args.content)
            elif args.type == "fix":
                cm.record_fix(args.content)
            else:
                cm.record_note(args.content)
            
            output_json({"status": "success", "type": args.type})
        ...
        # Use stderr for errors so JSON stdout isn't polluted
